...
def test(model, test_input_handle, configs, itr):
    ...
    res_path = os.path.join(configs.gen_frm_dir, str(itr) + '_gaussian_attacker')
    os.mkdir(res_path)
    avg_mse = 0
    batch_id = 0
    img_mse, ssim, psnr = [], [], []
    ...
    while (test_input_handle.no_batch_left() == False):
        ...
        new_test_dat = noise(torch.from_numpy(test_dat), eps=0.3)
        l2 += torch.dist(torch.from_numpy(test_dat), new_test_dat)

        new_test_dat = new_test_dat.numpy()
        ...
        if batch_id <= configs.num_save_samples:
            ...
            for i in range(configs.total_length):
                ...
                # save the gaussian attacked image
    ...
    l2 = l2 / batch_id
    print('l2 with gaussian noise attack on the test images: {}'.format(l2)) 
